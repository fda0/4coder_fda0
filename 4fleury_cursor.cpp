//
//~ NOTE(fda0): Cursor code taken from Ryan Fleury's layer and slightly modified by fda0
//


static b32 fda0_buffer_is_scrolling;
                         ...
                         Rect_f32 *rect, Rect_f32 *last_rect, Rect_f32 target)
{
    *last_rect = *rect;
    ...
    b32 should_animate_cursor = !global_battery_saver && !fda0_buffer_is_scrolling;
...
inline ARGB_Color
get_argb_set_falpha(ARGB_Color color, f32 alpha)
{
    alpha *= 255;
    clamp(0.f, alpha, 255.f);
    
    color &= 0x00FFFFFF;
    color |= (((ARGB_Color)alpha) << 24);
    
    return color;
}

static void
fda0_render_cursor(Application_Links *app, View_ID view_id, b32 is_active_view,
                   ...
                   f32 roundness, f32 outline_thickness, Frame_Info frame_info)
{
    ...
    // NOTE(fda0): Set normal/command mode color and lower opacity if recording macro
    ARGB_Color cursor_color;
    {
        if (!fda0_command_mode) { cursor_color = finalize_color(defcolor_cursor, 0); }
        else                    { cursor_color = finalize_color(defcolor_cursor, 1); }
        
        if (global_keyboard_macro_is_recording)
        {
            cursor_color &= 0x00ffffff;
            cursor_color |= 0xaa00006f;
        }
    }
    ...
    if(!has_highlight_range)
    {
        for(int i = 0; i < 1/*global_cursor_count*/; ++i)
        {
            ...
            // NOTE(rjf): Draw cursor.
            {
                ...
                {
                    F4_RenderCursorSymbolThingy(app, global_cursor_rect, roundness, 4.f, cursor_color, cursor_open);
                }
                
                // NOTE(rjf): Draw cursor glow (because why the hell not).
                for(int glow = 0; glow < 20; ++glow)
                {
                    f32 alpha = 0.1f - (glow*0.015f); // (glow*0.005f) <- in power mode
                    if(alpha > 0)
                    {
                        ...
                        F4_RenderCursorSymbolThingy(app, glow_rect, roundness + glow*0.7f, 2.f,
                                                    get_argb_set_falpha(cursor_color, alpha), cursor_open);
                    }
                    else
                    {
                        break;
            ...
            f32 mark_thickness = 1.0f;
            f32 mark_alpha = 0.5f;
            
            if (fda0_command_mode)
            {
                mark_thickness += 2.f;
                mark_alpha += 0.2f;
            }
            
            F4_RenderCursorSymbolThingy(app, global_mark_rect, roundness, mark_thickness,
                                        get_argb_set_falpha(cursor_color, mark_alpha), !cursor_open);
        }
    }
